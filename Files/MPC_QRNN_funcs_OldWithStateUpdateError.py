...
def start_QRNN_MPC_LBFGSB(prob_vars, env, seed, model_QRNN, replay_buffer_QRNN, optimizer_QRNN, use_sampling, use_mid):
    ...
    for episode in range(prob_vars.max_episodes):
        ...
        if episode <= 0: #10:
            if prob_vars.prob == "CartPole":
                actions = np.random.randint(0, 2, (prob_vars.horizon))
            elif prob_vars.prob == "Acrobot" or prob_vars.prob == "MountainCar":
                actions = np.random.randint(0, 3, (prob_vars.horizon))
            elif prob_vars.prob == "LunarLander":
            ...
            # if prob == "CartPole":
            #     actions = np.random.randint(0, 2, (1, horizon))
            # elif prob == "Acrobot" or prob == "MountainCar":
            #     actions = np.random.randint(0, 3, (1, horizon))
            # elif prob == "LunarLander":
            #     actions = np.random.randint(0, 4, (1, horizon))
            # elif prob == "PandaReacher" or prob == "MuJoCoReacher" or prob == "PandaPusher" or prob == "MuJoCoPusher" or prob == "LunarLanderContinuous":
            #     actions = np.random.uniform(action_low, action_high, (1, action_dim*horizon))
            # else: # Pendulum, MountainCarContinuous, Pendulum_xyomega
            #     actions = np.random.uniform(action_low, action_high, (1, horizon))
        ...
        for step in range(prob_vars.max_steps):
            ...
            for rep in range(prob_vars.nb_reps_MPC):
                iter_rep = minimize(fun=choose_action_func_LBFGSB,
                        x0 = actions,
                        jac=True,
                        args=(prob_vars, state, use_sampling, use_mid, model_QRNN), 
                        method='L-BFGS-B',

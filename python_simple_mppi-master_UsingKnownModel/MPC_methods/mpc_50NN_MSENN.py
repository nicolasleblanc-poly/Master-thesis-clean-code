import numpy as np
import torch

def mpc_50NN_MSENN_func(prob_vars, sim_states, particles): # , use_LBFGSB=False
    horizon = prob_vars.horizon
    num_particles = prob_vars.num_particles
    action_dim = prob_vars.action_dim
    model_state = prob_vars.model_state
    costs = torch.zeros(prob_vars.num_particles)
    
    for h in range(horizon):
        if prob_vars.use_ASNN and h == horizon-1: # Replace last action column with ASGNN predictions
            # Use ASGNN to generate last action/actions based of last non-terminal state
            # if prob == "CartPole" or prob_vars.prob == "Acrobot" or prob_vars.prob == "LunarLander" or prob_vars.prob == "MountainCar": # Discrete actions

            #     action_probs = model_ASN(sim_states, torch.tensor(prob_vars.goal_state, dtype=torch.float32))
                
            #     for loop_iter in range(num_particles):
            #         particles[loop_iter, -1] = np.random.choice(nb_actions, 1, p=action_probs[loop_iter].detach().numpy())

            # Continuous actions
            action_mus, action_sigmas = prob_vars.model_ASNN(torch.tensor(sim_states, dtype=torch.float32), torch.tensor(prob_vars.goal_state, dtype=torch.float32))

            for loop_iter in range(num_particles):
                particles[loop_iter, -action_dim:] = np.random.normal(action_mus.detach().numpy()[loop_iter], action_sigmas.detach().numpy()[loop_iter])
        
        particles_t_array = particles[:, h]
        actions = torch.tensor([particles_t_array], dtype=torch.float32).reshape(len(particles),action_dim)
        
        sim_states = torch.clip(sim_states, prob_vars.states_low, prob_vars.states_high)
        actions = actions.clip(prob_vars.action_low, prob_vars.action_high)

        next_states = model_state(sim_states, actions)

        # Update state and accumulate cost
        sim_states = next_states
        sim_states = torch.clip(sim_states, prob_vars.states_low, prob_vars.states_high)
        
        # if prob_vars.prob == "PandaReacher" or prob_vars.prob == "MuJoCoReacher" or prob_vars.prob == "PandaPusher" or prob_vars.prob == "MuJoCoPusher" or prob_vars.prob == "PandaReacherDense" or prob_vars.prob == "PandaPusherDense":
        #     costs += prob_vars.compute_cost(prob_vars.prob, next_states, h, horizon, actions, prob_vars.goal_state)
            
        # else:
        if h == horizon-1:
            # costs += prob_vars.terminal_cost_weight * prob_vars.compute_terminal_cost(prob_vars.prob_name, next_states).sum() #.sum(dim=1)#item() # h, horizon,
            costs += prob_vars.compute_terminal_cost(prob_vars.prob_name, next_states)

        else:

            print("next_states.shape ", next_states.shape, "\n")

            print("costs.shape ", costs.shape, "\n")


            print("prob_vars.stage_cost_weight.shape ", prob_vars.stage_cost_weight.shape, "\n")
            print("prob_vars.compute_state_cost(prob_vars.prob_name, next_states).shape ", prob_vars.compute_state_cost(prob_vars.prob_name, next_states).shape, "\n")
            # print("prob_vars.stage_cost_weight * prob_vars.compute_state_cost(prob_vars.prob_name, next_states).shape ", prob_vars.stage_cost_weight * prob_vars.compute_state_cost(prob_vars.prob_name, next_states).shape, "\n")
            # print("prob_vars.stage_cost_weight * prob_vars.compute_state_cost(prob_vars.prob_name, next_states).shape ", prob_vars.stage_cost_weight * prob_vars.compute_state_cost(prob_vars.prob_name, next_states).shape, "\n")
            # print("prob_vars.stage_cost_weight * prob_vars.compute_state_cost(prob_vars.prob_name, next_states).sum().shape ", prob_vars.stage_cost_weight * prob_vars.compute_state_cost(prob_vars.prob_name, next_states).sum().shape, "\n")
            
            # print("(prob_vars.stage_cost_weight * prob_vars.compute_state_cost(prob_vars.prob_name, next_states)).sum() ", (prob_vars.stage_cost_weight * prob_vars.compute_state_cost(prob_vars.prob_name, next_states)).sum(), "\n")
            
            # costs += prob_vars.stage_cost_weight * prob_vars.compute_state_cost(prob_vars.prob_name, next_states).sum() #(dim=1) #.item() # h, horizon,
            costs += prob_vars.compute_state_cost(prob_vars.prob_name, next_states)

            # costs += prob_vars.compute_cost(prob_vars.prob, next_states, h, horizon, actions) # h, horizon,

    return costs


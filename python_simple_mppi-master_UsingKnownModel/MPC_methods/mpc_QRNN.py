...
def mpc_QRNN_func(prob_vars, sim_states, particles): # , use_LBFGSB=False
    ...
    for h in range(horizon):
        ...
        if prob_vars.use_ASNN and h == horizon-1: # Replace last action column with ASGNN predictions
            ...
            # action_probs = prob_vars.model_ASNN(sim_states, torch.tensor(prob_vars.goal_state, dtype=torch.float32))
        ...
        predicted_quantiles = model_state(sim_states, actions)

        if prob_vars.use_sampling:
            # '''

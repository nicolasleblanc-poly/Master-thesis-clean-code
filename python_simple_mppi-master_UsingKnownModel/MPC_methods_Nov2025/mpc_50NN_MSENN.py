...
def mpc_50NN_MSENN_func(prob_vars, sim_states, particles, use_ASNN, model_state, use_sampling, use_mid, model_ASNN): # , use_LBFGSB=False
    ...
    for h in range(horizon):
        if use_ASNN and h == horizon-1: # Replace last action column with ASNN predictions
            ...
            if prob_vars.prob == "CartPole" or prob_vars.prob == "Acrobot" or prob_vars.prob == "LunarLander" or prob_vars.prob == "MountainCar": # Discrete actions
                ...
                action_probs = model_ASNN(sim_states, torch.tensor(prob_vars.goal_state, dtype=torch.float32))
                ...
                action_mus, action_sigmas = model_ASNN(torch.tensor(sim_states, dtype=torch.float32), torch.tensor(prob_vars.goal_state, dtype=torch.float32))
        ...
        if prob_vars.prob == "PandaReacher" or prob_vars.prob == "MuJoCoReacher" or prob_vars.prob == "PandaPusher" or prob_vars.prob == "MuJoCoPusher" or prob_vars.prob == "PandaReacherDense" or prob_vars.prob == "PandaPusherDense":
            ...
            if prob_vars.prob=="Cartpole_TrueMPC" or prob_vars.prob=="Pendulum_TrueMPC":
                if h == horizon-1:
                    ...
                    costs += prob_vars.compute_terminal_cost(prob_vars.prob, next_states)

                else:
                    costs += prob_vars.compute_state_cost(prob_vars.prob, next_states)
            else:
